...
if (isset($_POST['save_stock'])) {
    ...
    if (mysqli_query($conn, $insert_batch)) {
        ...
        mysqli_query($conn, "INSERT INTO stock (item_id, batch_id, location_id, quantity) VALUES ('$item_id', '$new_batch_id', '$location_id', '$quantity')");
...
if (isset($_POST['qr_code'])) {
    ...
    if (filter_var($code, FILTER_VALIDATE_URL)) {
        $message = "⚠️ Scanned a Website Link! Use a Text-Only QR.";
        $message_type = "error";
    } else {
...
<head>
    ...
    <style>
        ...
        .scanner-wrapper { width: 100%; max-width: 300px; height: 300px; background: #000; margin: 0 auto 20px auto; border-radius: 12px; overflow: hidden; display: none; border: 4px solid #4f46e5; }
        ...
        .float-home-btn { position: fixed; bottom: 20px; right: 20px; background: #1e1b4b; color: white; width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 15px rgba(0,0,0,0.3); text-decoration: none; font-size: 24px; z-index: 9999; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="container">
        <?php if ($message): ?>
            <div class="alert-box <?php echo ($message_type == 'success') ? 'alert-success' : 'alert-error'; ?>"><?php echo $message; ?></div>
        <?php endif; ?>

        <?php if (!$scanned_item): ?>
            <div class="scanner-container">
                <form method="POST" id="scanForm"><input type="hidden" name="qr_code" id="hiddenCode"></form>
                <div class="scanner-wrapper" id="scannerBox"><div id="reader" style="width:100%;height:100%;"></div></div>
            ...
            <div class="result-card">
                ...
                <form method="POST" style="margin-top:20px;">
                    ...
                    <select name="location_id" class="modern-select" required>
                        ...
                        $item_category = $scanned_item['category'];
                        
                        // If it's a Drink, prefix must be '0-'. If it's Dry, prefix must be '1-'.
                        $location_prefix = ($item_category === 'Drinks') ? '0-' : '1-';
                        
                        $l_sql = mysqli_query($conn, "SELECT * FROM locations WHERE location_code LIKE '$location_prefix%'");
                        
                        if(mysqli_num_rows($l_sql) > 0) {
                            while ($l = mysqli_fetch_assoc($l_sql)) {
                                echo "<option value='" . $l['location_id'] . "'>📍 " . $l['location_code'] . " (" . $l['description'] . ")</option>";
                            }
                        } else {
                            echo "<option value='' disabled selected>⚠️ No $item_category locations available! (Add in Admin)</option>";
                        }
                        ?>
                    </select>
    ...
    <script>
        function startScan() {
            ...
            html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } },
                ...
                (errorMessage) => { }
            ).catch(err => { alert("Camera Error: " + err); document.getElementById("scannerBox").style.display = "none"; document.getElementById("scanBtn").style.display = "inline-block"; });
        }
    </script>
